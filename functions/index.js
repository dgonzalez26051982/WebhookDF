...
server.post("/webhookmx", (req, res) => {
   ...
   // console.log(JSON.stringify(req.body.originalDetectIntentRequest));
...
server.post("/webhookbr", (req, res) => {
   ...
   // let d = new Date();
    ...
    res.setHeader('Content-Type', 'application/json');
    //******** */
    let mxTime = new Date().toLocaleString("en-MX", {timeZone: "America/sao_paulo"});
        mxTime = new Date(mxTime);
...
console.log("Seg OK");
    ...
    if (contexto === "input.welcome") {
        ...
        textoEnviar = 'São '+HoraMinutos.toString().substr(0, 2) + ' horas, com '+HoraMinutos.toString().substr(3, 2)+' minutos.';
        resultado = SAGACHDialogLib.respuestaBasica(textoEnviar,req.body.responseId,contexto);
        ...
        textoEnviar = 'Estamos no dia ' + fechaArray[2]+ '  do mês '+fechaArray[1]+' do ano ' +fechaArray[0];
        resultado = SAGACHDialogLib.respuestaBasica(textoEnviar,req.body.responseId,contexto);
        ...
        SAGACHDialogLib.sendEmail();
        console.log("++++enviado");
        resultado = SAGACHDialogLib.respuestaBasica("As informações foram enviadas para o seu email.",req.body.responseId,contexto);
        ...
        respuestaEnviada=true;
        /************** input.welcome */
        const reqUrl = encodeURI('http://api.weatherstack.com/current?access_key=cd2502e876a5be445c23705f8b809a4a&query=Brasil');
        SAGACHDialogLib.leerURLpromise(reqUrl).then((respuesta)=> {
            let resultado;
            textoEnviar="Na cidade de Brasília a temperatura é: "+respuesta.current.temperature+ " graus";
            resultado = SAGACHDialogLib.respuestaBasica(textoEnviar,req.body.responseId,contexto);
            res.json(resultado);
            MongoLib.saveInteractionBR(req.body.responseId,
                req.body.queryResult.queryText,
                contexto,
                textoEnviar,
                req.body.queryResult.intent.name,
            ...
            respuestaEnviada=false;
            console.log("error capturado en promise"+error);
            textoEnviar="Sinto muito, mas não consigo conectar ao servidor externo";
            res.json(SAGACHDialogLib.respuestaBasica(textoEnviar,req.body.responseId,contexto))
...
const local = false//para ejecutar servidor local debe estar en true de lo contrario false
if (local) {
